...
const OrganismTrendingNow: React.FC<OrganismTrendingNowProps> = ({events = defaultTrendingEvents}) => {
  ...
  return (
    <section className="w-full mt-0 md:mt-5 lg:mt-0">
      <div className="mb-4 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button type="button" variant="outline" size="icon-sm" className="rounded-full" onClick={scrollPrev} disabled={!canScrollPrev} aria-label="Previous trending events">
            <ChevronLeft className="h-4 w-4"/>
          </Button>
          <Button type="button" variant="outline" size="icon-sm" className="rounded-full" onClick={scrollNext} disabled={!canScrollNext} aria-label="Next trending events">
      ...
      <div className="overflow-hidden" ref={emblaRef}>
        <div className="-ml-3 flex">
          {
            events.map((event) => (
              <div key={event.id} className="min-w-0 flex-[0_0_100%] pl-3 md:flex-[0_0_50%]">
                <article className="relative h-72 overflow-hidden rounded-3xl">
                  <Image src={event.image} alt={event.title} fill className="object-cover" sizes="(min-width: 768px) 50vw, 100vw"/>
                  ...
                  <div className="absolute bottom-0 z-10 flex w-full flex-col gap-3 p-6">
                    ...
                    <AtomText type="sub-title" text={event.title} extraClass="font-bold text-white leading-tight"/>
